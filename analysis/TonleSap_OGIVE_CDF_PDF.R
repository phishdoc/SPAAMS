...
# 
# Last Modified by: Mark Yamane, 2023-11-2
#---
...
ggsave(paste0(prefix,"_Sv_12hour_Plot_stn1.png"), plot = Sv_time_plot_stn1, width =10, height =6, units = c("in"), dpi = 600) #saves the graph as a png
...
ggsave(paste0(prefix,"_Sv_12hour_Plot_stn2_only.png"), plot = Sv_time_plot_stn2,  width =10, height =6, units = c("in"), dpi = 600) #saves the graph as a png
message("Exported stn2 12hour plot: Sv_12hour_Plot_stn2.png")
...
Sv_hourly_stn2<- ggplot(exports_stn2, aes(x = Date_time, y = Sv_mean)) + geom_point() + 
  geom_vline(xintercept = (exports_stn2$date_column[exports_stn2$peak_status == "Peak"]), color = "darkgrey") +
  geom_smooth(formula = y ~ x, method = loess, se = T) + 
